...
'''
from QANet.main_debug_cla_ver646 import valid, test,train
flags = tf.flags
...
answer_dir = "./answers/ver646"
model_dir = "./models/ver646"

train_dir = "models"
model_name = "ver646"
answer_file = os.path.join(answer_dir, "testa_v646.txt")
valid_file = os.path.join(answer_dir, "valid_v646.txt")
...
def main(_):
    ...
    if args.mode == "valid":
        word_mat,counter,_ = prepro(646)
        valid(config,word_mat,counter)
    elif args.mode == "test":
        word_mat,counter,_ = prepro(646,type=1,input=args.input)
        test(config,word_mat,counter)
    elif args.mode == "train":
        word_mat, train_counter, w2id = prepro(646, type=2)
        _, dev_counter, _ = prepro(646, type=0, embedding_table=w2id)
