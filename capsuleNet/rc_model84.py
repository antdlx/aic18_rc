...
from capsuleNet.layers.basic_rnn import rnn
from capsuleNet.layers.caps_layer import routing
from capsuleNet.layers.caps_layer import softmax
from capsuleNet.layers.caps_layer import concat_capsules
from capsuleNet.layers.match_layer import AttentionFlowMatchLayer
from capsuleNet.layers.match_layer import SelfMatchingLayer
import numpy as np
...
class RCModel(object):
    ...
    def __init__(self, vocab, args):
        ...
        # 全局步数
        self.global_steps = tf.get_variable('global_step', shape=[], dtype=tf.int32,
    ...
    def _build_graph(self):
        ...
        self.logger.info('building graph used: {}  secs'.format(time.time() - start_t))
    ...
    def _create_train_op(self):
        self.learning_rate = tf.train.cosine_decay_restarts(self.learning_rate, self.global_steps, 2500)
        ...
        if self.optim_type == 'adagrad':
            ...
            raise NotImplementedError('can not support this optimizer: {}'.format(self.optim_type))
    ...
    def _train_epoch(self, train_batches, dropout_keep_prob):
        ...
        for bitx, batch in enumerate(train_batches, 1):
            ...
            if log_every_n_batch > 0 and bitx % log_every_n_batch == 0:
                ...
                #     eval_batches = data.gen_mini_batches('dev', 100, pad_id, shuffle=False)
                #     self.evaluate_acc(eval_batches)
                    self.save("../data/v81/models/", 'DEBUG_100')
                self.logger.info('From {} to {} batch average train loss is {}'.format(
    ...
    def train(self, data, epochs, batch_size, save_dir, save_prefix,
        ...
        for epoch in range(1, epochs + 1):
            self.logger.info('The {} time to train model '.format(epoch))
            train_batches = data.gen_mini_batches('train', batch_size, pad_id, shuffle=True)
            train_loss = self._train_epoch(train_batches, dropout_keep_prob)
            self.logger.info('This batch average train loss {} is {}'.format(epoch, train_loss))
    ...
    def evaluate_acc(self, eval_batches):
        ...
        if len(pred_answers) > 0:
            ...
            acc = 0
        self.logger.info('dev acc {}'.format(acc))
        self.logger.info('dev average loss is {}'.format(1.0 * total_loss / total_num))
    ...
    def evaluate(self, eval_batches, result_dir=None, result_prefix=None):
        ...
        #     if pred_labels[i] == int(true_answers[i]):
        #         count += 1
        ...
        if result_dir is not None and result_prefix is not None:
            ...
            with open(result_file, 'w',encoding='utf8',errors='ignore') as fout:
                for pred_answer in pred_answers:
                    fout.write(json.dumps(pred_answer, ensure_ascii=False) + '\n')

            self.logger.info('save {} ans to {}'.format(result_prefix, result_file))
    ...
    def save(self, model_dir, model_prefix):
        ...
        self.logger.info('save model to {}, prefix is {}.'.format(model_dir, model_prefix))
    ...
    def restore(self, model_dir, model_prefix):
        ...
        self.logger.info('reload {} model, prefix {}'.format(model_dir, model_prefix))
