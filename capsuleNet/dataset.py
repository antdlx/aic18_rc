...
class BRCDataset(object):
    ...
    def __init__(self, max_p_len, max_q_len,
        ...
        if train_files:
            ...
            self.logger.info('train {} ques.'.format(len(self.train_set)))

        if dev_files:
            for dev_file in dev_files:
                self.dev_set += self._load_dataset(dev_file)
            self.logger.info('dev {} ques'.format(len(self.dev_set)))

        if test_files:
            for test_file in test_files:
                self.test_set += self._load_dataset(test_file,test=True)
            self.logger.info('test {} ques'.format(len(self.test_set)))

    def _load_dataset(self, data_path, sampling=False,test=False):
        '''
        加载数据集
        '''
        with open(data_path, encoding='utf-8') as fin:
            ...
            for lidx, line in enumerate(fin):
                ...
                if test != True:
                    ...
                    if len(sample['segmented_passage']) > self.max_p_len:
                        filter_long_para += 1
                        sample['segmented_passage'] = sample['segmented_passage'][:self.max_p_len]
                    if len(sample['segmented_query']) > self.max_q_len:
                        filter_long_query += 1
                        sample['segmented_query'] = sample['segmented_query'][:self.max_q_len]
                    if len(sample['segmented_query']) == 0:
                        filter_zero_query += 1
                        sample['segmented_query'].append(['Empty'])
                if len(sample['segmented_passage']) > self.max_p_len:
                    filter_long_para += 1
                    continue
                if len(sample['segmented_query']) > self.max_q_len:
        ...
        print('passage too long :', filter_long_para, 'query filte:', filter_long_query + filter_zero_query)
        return data_set
    ...
    def _one_mini_batch(self, data, indices, pad_id):
        ...
        for sidx, sample in enumerate(batch_data['raw_data']):
            ...
            if len(sample['alternatives_token_ids']) < 3:
                for i in range(0,3-len(sample['alternatives_token_ids'])):
                    sample['alternatives_token_ids'].append([0])
    ...
    def gen_mini_batches(self, set_name, batch_size, pad_id, shuffle=True):
        ...
        if set_name == 'train':
            ...
            raise NotImplementedError('no such {} set'.format(set_name))
        data_size = len(data)
