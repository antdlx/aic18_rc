    ...
    def forward(self,trainable):
        ...
        with tf.variable_scope("Input_Embedding_Layer"):
            ...
            ch_emb = conv(ch_emb, d,
                bias = True, activation = tf.nn.relu, kernel_size = 1, name = "char_conv", reuse = None)
            qh_emb = conv(qh_emb, d,
                bias = True, activation = tf.nn.relu, kernel_size = 1, name = "char_conv", reuse = True)
        ...
        with tf.variable_scope("Output_Layer"):
            ...
            after_pool = tf.layers.max_pooling1d(inputs=after_conv,pool_size=2,strides=1)
            pool_flat = tf.reshape(after_pool, [-1, 99 * 32])
