    ...
    def load_pretrained_embeddings(self, embedding_path):
        ...
        for token in self.token2id.keys():
            if token in trained_embeddings:
                ...
                count +=1
        print('init vec num{}'.format(count))

    def load_pretrained_embeddings_txt(self, embedding_path):
        """
        根据文件路径，加载预训练的词向量
        不在该词向量集中的词语将被过滤
        Args:
        ...
        word2id = {}
        cnt = -1
        fr = open(embedding_path, 'r', encoding='utf8', errors='ignore')
        ...
        for token in word2id.keys():
            if token not in self.token2id:
                continue
            trained_embeddings[token] = embedding_table[word2id[token]]
            if self.embed_dim is None:
        ...
        for token in self.token2id.keys():
            if token in trained_embeddings:
                self.embeddings[self.get_id(token)] = trained_embeddings[token]
                count +=1
        print('word vec init num{}'.format(count))
